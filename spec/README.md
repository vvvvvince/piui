# spec/ — the piui specification corpus

This directory is the normative specification for `piui`, a self-hosted web front end for
[`pi`](https://www.npmjs.com/package/@earendil-works/pi-coding-agent). It is meant to be handed
to a coding agent (or a developer) to implement. The implementation plan derived from it lives
in [`../plan/`](../plan/README.md); the project entry point is [`../README.md`](../README.md).

**This file documents the corpus itself** — reading order, per-task context budget, frontmatter
schema, and the conventions the spec files use. It is spec meta-documentation, not product
documentation: everywhere the spec files say "the README MUST state …" they mean the *product*
README at the repository root, which is an M7 deliverable.

## Feature summary

| # | Feature | Spec |
|---|---------|------|
| 1 | **Profiles** — AGENTS.md + skills + tools + opt-in persistent memory | [03-profiles.md](03-profiles.md) |
| 2 | **Authentication** — hardcoded `test`/`test`, pluggable later | [06-auth.md](06-auth.md) |
| 3 | **Workspaces** — a folder the agent works in | [04-workspaces.md](04-workspaces.md) |
| 4 | **Chat mode** — model only, web search, no memory/profile/persona | [07-chat-mode.md](07-chat-mode.md) |
| 5 | **Agent mode** — model + profile + workspace, full agentic loop | [08-agent-mode.md](08-agent-mode.md) |
| 6 | **Skills & tools management UI** | [05-skills-and-tools.md](05-skills-and-tools.md) |

> **Start here:** [00-overview.md](00-overview.md), then the
...
> [20-development-method.md](20-development-method.md) before writing any code. The
...
1. [00-overview.md](00-overview.md) — goals, non-goals, glossary, architecture
2. [01-architecture.md](01-architecture.md) — stack, processes, repo layout, pi integration
3. [02-data-model.md](02-data-model.md) — storage layout, DB schema, TypeScript types
4. [03-profiles.md](03-profiles.md)
5. [04-workspaces.md](04-workspaces.md)
6. [05-skills-and-tools.md](05-skills-and-tools.md)
7. [06-auth.md](06-auth.md)
8. [07-chat-mode.md](07-chat-mode.md)
9. [08-agent-mode.md](08-agent-mode.md)
10. [09-api.md](09-api.md) — REST + SSE contract (authoritative)
11. [10-frontend.md](10-frontend.md) — screens, components, UX states
12. [11-security.md](11-security.md)
13. [12-milestones.md](12-milestones.md) — build order + acceptance criteria
14. [13-open-questions.md](13-open-questions.md)
15. [14-credentials.md](14-credentials.md) — provider credential management (Q1 = B)
16. [15-commands-and-input.md](15-commands-and-input.md) — slash commands, prompt
    templates, pi-TUI input parity (Q2)
17. [16-extensions.md](16-extensions.md) — extensions: global install, per-profile
    disable, no approval gates (Q3)
18. [17-memory.md](17-memory.md) — memory file format + phased evolution (Q4 = D)
19. [18-multi-user.md](18-multi-user.md) — authorization model: ownership + roles
    enforced from V1, real multi-user in V2 (Q7 = C)
20. [19-deployment.md](19-deployment.md) — Dockerfile + docker-compose stack; the
    container is the isolation model (Q10 = A)
21. [20-development-method.md](20-development-method.md) — **test-driven development**
    (requirement R1): the loop, spec-as-test-backlog, and the M0 test seams

**[decisions.md](decisions.md)** — binding answers to the open questions. Read this
first; it overrides anything it contradicts.

## Spec frontmatter

Every file in `spec/` **except this one** starts with YAML frontmatter so a tool or an agent can
navigate without reading everything. (This file is meta-documentation *about* the corpus, not a
member of it, so it carries none — a frontmatter validator must skip `README.md`.)
...
- The validator globs `spec/*.md` only. Plan files under `../plan/` carry a *similar* but
  separate frontmatter schema and reference spec ids through `spec_refs`, precisely so they
  never take part in the `depends_on` / `required_by` symmetry rule above.
